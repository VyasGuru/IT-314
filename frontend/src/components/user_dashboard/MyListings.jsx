      ...
      {listings.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {listings.map((listing) => (
            <div key={listing.listingId} className="bg-white rounded-xl overflow-hidden border border-gray-200 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                <div className="relative w-full h-48 overflow-hidden group">
                    <img src={listing.property.images[0]} alt={listing.property.title} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
                    <span className={`absolute top-4 right-4 px-4 py-2 rounded-full text-xs font-semibold uppercase bg-white shadow-md border-2 ${listing.status === 'active' || listing.status === "verified" ? 'text-green-600 border-green-600' : 'text-amber-600 border-amber-600'}`}>
                        {listing.status}
                    </span>
                </div>
                <div className="p-6">
                    <div className="flex justify-between items-start mb-3">
                        <h3 className="text-lg font-semibold text-gray-800 flex-1">{listing.property.title}</h3>
                    ...
                    <p className="text-2xl font-bold text-blue-600 mb-3">${listing.property.price.toLocaleString()}</p>
                </div>
            </div>
          ))}
        </div>
