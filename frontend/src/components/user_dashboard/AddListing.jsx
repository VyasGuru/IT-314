...
import { BedDouble, Bath, Car, Utensils, Wifi, ShieldCheck, Zap, Leaf, Building, Play, Sofa, Home, Milestone, Calendar, MapPin, ImagePlus, Upload, Loader2, AlertCircle, CheckCircle2, IndianRupeeIcon, ChevronDown } from 'lucide-react';
...
const AddListing = () => {
    ...
    const handleLocationChange = (e) => {
        const { name, value } = e.target;
        // For zipCode, only allow numbers and max 6 digits
        if (name === 'zipCode') {
    ...
    const validateForm = () => {
        ...
        // Validate Year Built (DD/MM/YYYY)
        if (!isValidDate(formData.yearBuild)) {
            return "Please enter a valid Year Built in DD/MM/YYYY format.";
        }

        if (!formData.propertyType) return "Please select a Property Type.";

        // Validate Zip Code
        if (!/^\d{6}$/.test(formData.location.zipCode)) {
        ...
        <div className="bg-slate-50 min-h-screen">
            <div className="container mx-auto p-4 sm:p-6 lg:p-8">
                <div className="max-w-4xl mx-auto">
                    <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 tracking-tight mb-8">Create a New Property Listing</h1>
                        ...
                        <div className="bg-white p-6 rounded-xl shadow-md">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="md:col-span-2">
                                    <textarea name="description" placeholder="Property Description" value={formData.description} onChange={handleChange} rows="4" className="w-full p-4 border rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500" required></textarea>
                                </div>
                                ...
                                <div className="relative">
                                    ...
                                    <select name="propertyType" value={formData.propertyType} onChange={handleChange} className="w-full pl-14 pr-10 py-2 border rounded-lg bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none" required>
                                ...
                                {Object.keys(formData.amenities).map(amenity => {
                                    ...
                                    return (
                                        <label key={amenity} className="flex items-center space-x-3 cursor-pointer">
                                            <input type="checkbox" name={amenity} checked={formData.amenities[amenity]} onChange={handleAmenitiesChange} className="h-5 w-5 rounded-md border-gray-300 text-blue-600 focus:ring-blue-500" />
